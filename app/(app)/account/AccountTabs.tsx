...
function CardHead({ title, desc, danger }: { title: string; desc: string; danger?: boolean }) {
  return (
    <Box sx={cardHeadSx}>
      <Typography
        component="h2"
        sx={{ ...serif, fontSize: "21px", fontWeight: 500, mb: 0.5, color: danger ? "error.main" : "text.primary" }}
      >
        {title}
      </Typography>
      <Typography sx={{ fontSize: "14px", color: "text.secondary", maxWidth: "60ch" }}>{desc}</Typography>
    </Box>
  );
}
...
function LabelledInput({
  ...
  return (
    <Box sx={{ display: "grid", gap: 0.875 }}>
      <Box component="label" htmlFor={id} sx={{ fontSize: "13px", fontWeight: 500, color: "text.secondary" }}>
        {label}
      </Box>
      <OutlinedInput
        id={id}
...
export default function AccountTabs({
  ...
  return (
    <>
      ...
      >
        <Box role="tablist" sx={{ display: "flex", gap: 0.5, overflowX: "auto", scrollbarWidth: "none" }}>
          {tabs.map((it) => {
            const on = tab === it.key;
            return (
              <Box
...
function ProfilePanel({
  ...
  return (
    <Box role="tabpanel" sx={cardSx}>
          ...
          <Box sx={{ display: "grid", gap: 2.75, minWidth: 0 }}>
            <Box sx={{ display: "grid", gap: 1.25 }}>
              <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
                <Button size="small" variant="outlined" sx={ghostSx} onClick={() => onToast(t("uploadToast"))}>
                  {t("upload")}
                </Button>
                <Button size="small" variant="outlined" sx={ghostSx} onClick={() => onToast(t("initialsToast"))}>
                  {t("useInitials")}
                </Button>
              </Box>
            ...
            <Box sx={{ display: "grid", gap: { xs: 2.25, sm: "18px" }, gridTemplateColumns: { sm: "1fr 1fr" } }}>
              <LabelledInput label={t("nameLabel")} value={name} onChange={setName} autoComplete="name" />
              <LabelledInput
                label={t("emailLabel")}
                type="email"
                value={email}
      ...
      <Box sx={cardFootSx}>
        <Box sx={{ fontSize: "13px", color: "text.secondary" }}>{dirty ? t("dirty") : t("clean")}</Box>
        <Button
          size="small"
          variant="contained"
          disabled={!dirty}
...
function SecurityPanel({ sessions, onToast }: { sessions: Session[]; onToast: (m: string) => void }) {
  ...
  return (
    <Box sx={{ display: "grid", gap: 2.25 }}>
      <Box sx={cardSx}>
        ...
        <Box sx={cardFootSx}>
          <Box sx={{ fontSize: "13px", color: "text.secondary", "& a": { color: "text.primary", borderBottom: "1px solid", borderColor: "divider" } }}>
            {t.rich("forgotHint", {
              link: (chunks) => (
      ...
      <Box sx={cardSx}>
            ...
            {sessions.map((s, i) => (
              ...
              >
                ...
                <Box sx={{ display: "grid", gap: "2px", minWidth: 0 }}>
                  <Box sx={{ color: "text.primary", fontWeight: 500, display: "flex", alignItems: "center", gap: 0.75, flexWrap: "wrap" }}>
                    {s.label}
                    {s.current ? (
                      <Box
                        component="span"
                ...
                {s.current ? (
                  ...
                  <Button size="small" variant="outlined" sx={ghostSx} onClick={() => onToast(t("disconnectedToast"))}>
                    {t("disconnect")}
                  </Button>
                )}
              </Box>
        ...
        <Box sx={cardFootSx}>
          ...
          <Button size="small" variant="outlined" sx={ghostSx} onClick={() => onToast(t("disconnectAllToast"))}>
            {t("disconnectAll")}
          </Button>
        </Box>
      </Box>
  ...
  return (
    <Box sx={{ display: "grid", gap: 2.25 }}>
      ...
      <Box
        sx={{
          ...cardSx,
          borderColor: "color-mix(in oklab, var(--mui-palette-error-main) 18%, var(--mui-palette-divider))",
          bgcolor: "color-mix(in oklab, var(--mui-palette-error-light) 30%, var(--mui-palette-background-paper))",
        }}
      >
        ...
        <DataRow
          ...
          action={
            <Button size="small" variant="outlined" sx={dangerGhostSx} onClick={() => setModalOpen(true)}>
              {t("deleteBtn")}
            </Button>
          }
        />
  ...
  return (
    ...
    >
      <Box sx={{ display: "grid", gap: 0.5 }}>
        ...
        <Typography sx={{ fontSize: "14px", color: "text.secondary", maxWidth: "56ch" }}>{body}</Typography>
      </Box>
...
function DeleteModal({
  ...
  const dotsSx = { display: "inline-flex", alignItems: "center", gap: 0.5, mr: "auto", ...mono, fontSize: "10.5px", color: "text.disabled" };
  const dot = (on: boolean) => (
    <Box component="span" sx={{ width: 6, height: 6, borderRadius: "999px", bgcolor: on ? "error.main" : "divider" }} />
  );
  ...
  return (
    ...
    >
      ...
      <Box sx={{ p: "12px 24px 6px 75px", fontSize: "14.5px", color: "text.secondary" }}>
        {step === 1 ? (
          <>
            <Typography sx={{ fontSize: "14.5px", color: "text.secondary" }}>{t("step1Intro")}</Typography>
            ...
            <Typography sx={{ fontSize: "14.5px", color: "text.secondary", mt: 1.25, "& a": { color: "text.primary", borderBottom: "1px solid", borderColor: "divider" } }}>
              {t.rich("step1Export", {
                link: (chunks) => (
          ...
          <>
            <Typography sx={{ fontSize: "14.5px", color: "text.secondary" }}>
              {t.rich("step2Prompt", {
                ...
                code: (chunks) => (
                  <Box
                    component="code"
                    sx={{ ...mono, textTransform: "none", fontSize: "12.5px", bgcolor: "background.sunk", px: 0.75, py: "1px", borderRadius: "4px", color: "text.primary" }}
                  >
                    {chunks}
                  </Box>
                ),
            ...
            <Box sx={{ mt: 2 }}>
              <Box component="label" htmlFor="del-confirm" sx={{ display: "block", fontSize: "13px", color: "text.primary", mb: 0.75 }}>
                {t("confirmLabel")}
              </Box>
              <OutlinedInput
                id="del-confirm"
